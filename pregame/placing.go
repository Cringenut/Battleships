	...
	for _, shipSize := range shipSizes {
		...
		for !placed {
			...
			if shipSize > 1 {
				...
				if !validEnd || !contains(ends, endInput) {
...
func isValidCoordinate(coord string) (int, int, bool) {
	...
	if len(coord) == 3 && coord[1] == '1' && coord[2] == '0' {
		...
		row = invertNumber(int(coord[1] - '0'))
	} else {
		return 0, 0, false
	}
...
func possibleEndCoords(row, col, length int) []string {
	...
	for _, d := range directions {
		...
		if valid {
			formattedEnd := fmt.Sprintf("%c%d", rune('A'+endCol), invertNumber(endRow)) // Ensure correct formatting
